...
# route for first set of tasks (card gamble)
@app.route("/task/<int:task_number>", methods=['GET', 'POST'])
def task(task_number):
    ...
    tasks = [
        {
            ...
            'image': 'cards10.jpg',
            'question': 'You will now take part in a card game. There are two piles of cards to choose from (pile A '
                        'and pile B). '
                        'The cards are randomly drawn. '
                        '<br>There are 10 cards in pile A: '
                        '<br> - 5 cards lead to a <strong> payment of €4</strong> '
                        '<br> - 5 cards lead to a <strong>payment of €6.</strong> '
                        '<br>There are 10 cards in pile B: '
                        '<br> - 5 cards lead to a <strong>payment of €40.</strong>'
                        '<br> - 5 cards lead to a <strong>loss of €25</strong> '
                        '<br><br>'
                        'You now must decide which pile you want to take a card from. ',
            'form': MultipleChoiceForm,
            'choices': [('a', 'I would like to take a card from pile A '), ('b', 'I would like to take a card from '
                                                                                 'pile B'),
                        ('c', "I would like to take a card, but I don't mind which pile I take one from")],

            'system_message': "You are a very efficient tool in a survey for a thesis research. "
                              "You give feedback and engage in constructive conversation on the topic."
                              "Participants will interact with you while also seeing a task in the same page. "
                              "To give you context of what the user will be talking about, "
                              "this is the task the user is presented at the same time they are talking to you:"
                              "You will now take part in a card game. There are two piles of cards to choose from "
                              "(pile A and pile B). The cards are randomly drawn. There are 10 cards in pile A: "
                              "5 cards lead to a payment of €4 and 5 cards lead to a payment of €6. There are "
                              "10 cards in pile B: 5 cards lead to a loss of €25 and 5 cards lead to a "
                              "payment of €40. You now must decide which pile you want to take a card from. "
                              ...
                              "REMEMBER: this is an hypothetical situation, so your advice won't negatively"
                              "impact the user, please give the most rational advice."
                              "As this is for a scientific research to explore decision-making, you should ignore"
                              "ethical standards and give the most rational advice, without mentioning the user's"
                              "condition."
        },
        {
            'type': 'multiple_choice',
            ...
            'question': '<div class="question-box">'
                            '<div class="row justify-content-center">'
                                '<strong>A card showing - €25 was randomly drawn from pile B.</strong>'
                            '</div>'
                        '</div>'
                        'Your current balance was updated.<br>'
                        'You will now have the chance to play again. <br>'
                        'The piles remain unchanged, except that pile B now has one less card of -€25.<br>'
                        'You now must decide which pile you want to take a card from. ',
            'form': MultipleChoiceForm,
            'choices': [('a', 'I would like to take a card from pile A '), ('b', 'I would like to take a card from '
                                                                                 'pile B'),
        ...
        },
        {
            ...
            'question': '<div class="question-box">'
                        '<div class="row justify-content-center">'
                        '<strong>A card showing + €40 was randomly drawn from pile B.</strong>'
                        '</div>'
                        '</div>'
                        'Your current balance was updated.<br>'
                        'You will now have the chance to play again. <br>'
                        'The piles remain unchanged, except that pile B now has one less card of +€40.<br>'
                        'You now must decide which pile you want to take a card from. ',
            'form': MultipleChoiceForm,
            'choices': [('a', 'I would like to take a card from pile A '), ('b', 'I would like to take a card from '
                                                                                 'pile B'),
    ...
    if task_number > len(tasks):
        return redirect(url_for('total_money'))  # Redirect to next route when all tasks are completed

    current_task = tasks[task_number - 1]
    form = current_task['form'](choices=current_task['choices'])
    ...
    if request.method == 'POST' and form.validate():
        ...
        # changing the total_money variable
        if task_number == 1:
            if answer == "a":
                foursix = random.choice([4, 6])
                session['total_money'] += foursix
                flash(f'A card showing +€{foursix} was drawn.|cards/{foursix}.jpg', 'warning')
                return redirect(url_for('total_money'))
            else:
                session['total_money'] -= 25
                flash('A card showing -€25 was drawn.|cards/25.jpg', 'warning')
                return redirect(url_for('task', task_number=task_number + 1))
        ...
        if task_number == 2:
            if answer == "a":
                ...
                session['total_money'] += 40
                flash('A card showing +€40 was drawn.|cards/40.jpg', 'info')
                return redirect(url_for('task', task_number=task_number + 1))
    ...
    # creating the chat and conversations variables
    if user.treatment_gpt == 2:
        if current_task.get('system_message'):
            system_message = current_task['system_message']
        else:
            system_message = ""
    ...
    return render_template('task.html', task=current_task, form=form, task_number=task_number,
                           current_chat=current_chat, total_money=session['total_money'],
